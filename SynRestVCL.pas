  ...
  begin
    ...
    begin
      case fParams[i].DataType of
        ftString,ftWideString:value:=QuotedStr(fParams[I].AsString);
      else
        value:=fParams[I].AsString;
      end;
    end;
    Result := StringToUTF8(StringReplace(UTF8ToString(Result), lParamName, value, [rfIgnoreCase]));
  end;
end;
      ...
      if (lRespObj.Kind = dvObject) then
        if (lRespObj.GetValueIndex('errorCode') > -1) then
          ...
          begin
            ...
            raise ESynException.CreateUTF8('Error' + sLineBreak + '%' + sLineBreak + 'from' + sLineBreak + '%', [lErrMsg, aSOAService]);
          end;
...
begin
  ...
  if not Assigned(lSQLTableJSON) then
    raise ESynException.Create(UTF8ToString(TSQLRestClientURI(fRestClient).LastErrorMessage));
...
begin
  ...
  begin
    ...
    begin
      ...
      if (lFieldDef.DataType <> SQLFieldTypeToVCLDB[lFieldType]) then
